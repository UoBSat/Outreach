...
def main_loop(display, joystick):
    ...
    while keep_running:
        ...
        if x > 0 and not limit_switches[INDEX_Y_POS].limit_triggered:
            step_delay = (1/x)*0.00045
            #print(x)
...
"""        if x != 0:
        ...
        if x > 0 and not limit_switches[INDEX_Y_POS].limit_triggered:
            delay = xHold*0.0009
            #print(x)
            stepper_1.step_clockwise(delay)
        elif x < 0 and not limit_switches[INDEX_X_NEG].limit_triggered:
            xHold = xHold*(-1)
            delay = xHold*0.0009
            #print(x)
            stepper_1.step_counterclockwise(delay)
        
        if y > 0 and not limit_switches[INDEX_Y_POS].limit_triggered:
            delay = yHold*0.0009
            #print(y)
            stepper_2.step_clockwise(delay)
        elif y < 0 and not limit_switches[INDEX_Y_NEG].limit_triggered:
            yHold = yHold*(-1)
            delay = yHold*0.0009
            #print(y)
            stepper_2.step_counterclockwise(delay)
        #Ends here """
